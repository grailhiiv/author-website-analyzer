...
The typed catalog in `src/lib/scoring/check-status-guidance.ts` provides deterministic author-facing guidance for every registered check when its result is `pass` or `unknown`. Passed results include check-specific confirmation details, a maintenance recommendation, and practical tips. Unknown results combine the saved reason code with a check-specific manual verification step and actions for improving evidence coverage before rescanning. This guidance is selected by check ID and result state; it is not generated by AI and does not affect scoring.
