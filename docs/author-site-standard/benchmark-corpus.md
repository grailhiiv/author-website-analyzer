...
Schema version 1.0.0 contains 31 cases and 41 scanned pages. It includes conventional and custom paths, structured-data discovery, local and hosted newsletters, ambiguous forms, article and event traps, malformed JSON-LD, incomplete crawl coverage, JavaScript shells, render failures, rendered pages with and without material evidence gain, lazy CDN cover images, Cloudflare-protected contact email addresses, author-name-only homepage headings, concise positioning copy outside an H1, author-photo filename and alt-text conventions, repeated-footer contact traps, multi-book What's New pages, semantic WordPress post markup, and route-priority conflicts between homepage/privacy roles and embedded page features. The live-regression cases retain paired negative safeguards for portrait-gallery image traps, malformed protected-email values, homepage identity text incorrectly treated as a descriptive headline, and generic author wording on non-portrait images.
